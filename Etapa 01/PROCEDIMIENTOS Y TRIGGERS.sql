...
GO
CREATE OR ALTER PROCEDURE SP_INSERTAR_PRODUCTO(@ID_PRODUCTO INT OUT,
		...
		BEGIN TRY
			BEGIN TRANSACTION
				IF(EXISTS(SELECT 1 FROM PRODUCTOS WHERE ID_PRODUCTO = @ID_PRODUCTO))
					BEGIN
						UPDATE PRODUCTOS SET CODIGO_BARRA = @CODIGO_BARRA,
				...
				ELSE
					BEGIN
						INSERT INTO PRODUCTOS(CODIGO_BARRA,ID_PROVEEDOR,NOMBRE_PROVEEDOR,DESCRIPCION,PRECIO_UNIDAD)
										VALUES(@CODIGO_BARRA,@ID_PROVEEDOR,@NOMBRE_PROVEEDOR,@DESCRIPCION,@PRECIO_UNIDAD)
...
GO
CREATE OR ALTER PROCEDURE SP_ELIMINAR_PRODUCTO(@ID_PRODUCTO INT OUT, @MSJ VARCHAR(200) OUT)
AS
	BEGIN
		BEGIN TRY
			BEGIN TRANSACTION
				...
				ELSE IF(NOT EXISTS(SELECT 1 FROM INVENTARIOS WHERE ID_INVENTARIO = @ID_INVENTARIO))
					BEGIN
						INSERT INTO INVENTARIOS(ID_PRODUCTO, DESCRIPCION, CANTIDA_INGRESADA, EXISTENCIA,
											   CANTIDAD_VENDIDA, PRECIO_UNIDAD)
...
GO
CREATE OR ALTER PROCEDURE SP_ELIMINAR_PRODUCTO_INVENTARIO(@ID_INVENTARIO INT OUT, @MSJ VARCHAR(200) OUT)
AS
	BEGIN
		BEGIN TRY
			BEGIN TRANSACTION	
...
GO
CREATE OR ALTER PROCEDURE SP_BUSCAR_PROVEEDOR(@ID_PROVEEDOR INT OUT)
AS
	BEGIN
